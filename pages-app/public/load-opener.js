...
function renderLoad(opened, payload) {
  ...
  app.innerHTML = `
    <main>
      ...
      <p class="lede">${escapeHtml(opened.bookmark.domain || "Keepseek Load")} · opened from one self-contained file.</p>
      <div class="proof">
        <div>
          <small>Root hash</small>
          <code>${escapeHtml(opened.manifest.hashing.root.slice(0, 20))}…</code>
        </div>
        <span>SHA-256 ✓</span>
      </div>
      ...
      <div class="actions">
        <a href="${escapeHtml(embeddedLink)}">Open in Keepseek</a>
        <button type="button" class="secondary" id="download-load">Download .muthur.load</button>
      </div>
    </main>
  `;
...
async function boot() {
  if (!payloadElement) {
    renderError("This open file is missing its embedded Load.");
    return;
  }
  ...
  try {
    ...
    if (payload.root) {
      ...
      if (opened.manifest.hashing.root !== payload.root) {
        throw new Error("Embedded Load root hash does not match this open file.");
      }
      renderLoad(opened, payload);
      return;
    }
    renderLoad(await openEmbeddedLoad(bytes, payload.fileName || "shared.muthur.load"), payload);
  } catch (error) {
    renderError(error instanceof Error ? error.message : "This open file could not verify its Load.");
  }
}
